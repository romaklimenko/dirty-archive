...
def media():
    ...
    for media in media_collection.find(query):
        ...
        print(
            f'{format_number(media_to_process - count + 1)} из {format_number(media_to_process)} ({round((media_to_process - count) / media_to_process * 100)}%)',
            url)
        count -= 1
        ...
        try:
            ...
            blob.upload_from_string(
                content, content_type=media['content_type'])
        except Exception as e:
